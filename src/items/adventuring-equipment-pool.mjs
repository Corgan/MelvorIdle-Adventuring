/**
 * AdventuringEquipmentPool - A pool of equipment that can drop from monsters.
 * Items in the pool are weighted and only drop if they haven't been dropped before.
 * Once dropped, the item's `dropped` flag is set to true, unlocking it in the Armory.
 */
export class AdventuringEquipmentPool extends NamespacedObject {
    constructor(namespace, data, manager, game) {
        super(namespace, data.id);
        this.manager = manager;
        this.game = game;
        this.name = data.name || 'Equipment Pool';
        
        // Store raw item data for later resolution in postDataRegistration
        this._itemData = data.items || [];
        this.items = []; // Array of { item: AdventuringBaseItem, weight: number }
    }
    
    /**
     * Resolve item references after all items are registered
     */
    postDataRegistration() {
        this.items = this._itemData.map(entry => {
            // Support both string shorthand and object with weight
            const id = typeof entry === 'string' ? entry : entry.id;
            const weight = typeof entry === 'string' ? 1 : (entry.weight !== undefined ? entry.weight : 1);
            const item = this.manager.baseItems.getObjectByID(id);
            
            if (!item) {
                console.warn(`[Adventuring] Equipment pool ${this.id}: Item not found: ${id}`);
                return null;
            }
            
            return { item, weight };
        }).filter(e => e !== null);
    }
    
    /**
     * Get all items that haven't dropped yet
     * @returns {Array<{item: AdventuringBaseItem, weight: number}>}
     */
    getAvailable() {
        return this.items.filter(entry => !entry.item.dropped);
    }
    
    /**
     * Check if any items in the pool are still available to drop
     * @returns {boolean}
     */
    hasAvailable() {
        return this.items.some(entry => !entry.item.dropped);
    }
    
    /**
     * Roll for a random item from the pool (only un-dropped items)
     * Uses weighted random selection.
     * @returns {AdventuringBaseItem|null} The selected item, or null if none available
     */
    roll() {
        const available = this.getAvailable();
        if (available.length === 0) return null;
        
        const totalWeight = available.reduce((sum, e) => sum + e.weight, 0);
        let roll = Math.random() * totalWeight;
        
        for (const entry of available) {
            roll -= entry.weight;
            if (roll <= 0) {
                return entry.item;
            }
        }
        
        // Fallback to last item (shouldn't happen with proper weights)
        return available[available.length - 1].item;
    }
    
    /**
     * Get progress information for UI display
     * @returns {{dropped: number, total: number, percent: number}}
     */
    getProgress() {
        const dropped = this.items.filter(e => e.item.dropped).length;
        const total = this.items.length;
        return {
            dropped,
            total,
            percent: total > 0 ? Math.round((dropped / total) * 100) : 0
        };
    }
}
